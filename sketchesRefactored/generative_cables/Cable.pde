class Cable {
  ...
  ArrayList<CablePoint> createCablePoints() {
    ...
    cablePoints.add(new CablePoint(startJack.x, startJack.y, true, true));

    float directDistance = dist(startJack.x, startJack.y, endJack.x, endJack.y);
    ...
    for (int i = 1; i < segmentCount; i++) {
      ...
      float x = lerp(startJack.x, endJack.x, t);
      float y = lerp(startJack.y, endJack.y, t);
    ...
    cablePoints.add(new CablePoint(endJack.x, endJack.y, true, true));
 ...
 // Enforce cable segment length constraints
  ...
  // Draw connectors at each end of the cable
  void drawConnectors() {
